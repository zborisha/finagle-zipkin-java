...
public abstract class FinagleUtil {
	...
	public static Service<Request, Response> getFinagleClientForServiceByName(String serviceName) {
		...
		Service<Request, Response> service = ClientBuilder
				.safeBuild(ClientBuilder.get().name("client-on-" + serverName).codec(com.twitter.finagle.http.Http.get().enableTracing(true))
						.hosts(address + ":" + port).failFast(false).tracer(zipkinTracer).hostConnectionLimit(1).retries(2));
		return service;
	}
